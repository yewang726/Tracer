...
class SphericalGM(QuadricGM):
	...
	def mesh(self, resolution):
		...
		theta = N.vstack(N.linspace(0., N.pi, resolution+1))
		phi = N.linspace(0., 2.*N.pi, resolution*2+1)
		
		x = self._rad * N.sin(theta) * N.cos(phi)
		y = self._rad * N.sin(theta) * N.sin(phi)
		z = N.tile(self._rad * N.cos(theta), (1, x.shape[1]))
		x, y, z = N.around(x, decimals=6), N.around(y, decimals=6), N.around(z, decimals=6)
		return x, y, z
		
	def get_fluxmap(self, eners, local_coords, resolution):
		'''
		Polar mesh for spherical surfaces
		'''

		ths_bin = N.linspace(0., N.pi, resolution+1)
		phis_bin = N.linspace(0., 2.*N.pi, resolution*2+1)
		ths = N.arccos(local_coords[2]/self._rad)
		phis = N.arctan2(local_coords[1], local_coords[0])
		phis[phis<0.] += 2.*N.pi
		eners = N.histogram2d(ths, phis, bins=[ths_bin, phis_bin], weights=eners)[0]
		ths_bin = N.vstack(ths_bin)
		dphi = phis_bin[1:]-phis_bin[:-1]
		areas = self._rad**2.*dphi*(N.cos(ths_bin[:-1])-N.cos(ths_bin[1:]))
		flux = eners/areas
